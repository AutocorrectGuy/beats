...
const TrackCard = ({ price, badge, bpm, hasAd, beatName, author, image }: TrackCardProps) => {
  return (
    <div className="w-full shadow-xl rounded-lg overflow-hidden p-3 hover:bg-neutral-600 hover:bg-opacity-25 border border-transparent hover:border-neutral-700 cursor-pointer">
      {/* Image */}
      ...
      <div className="py-2">
        {/* 1st row */}
        <div className="flex justify-start items-center gap-1 overflow-hidden font-medium">
          <div className="text-blue-500">€{price}</div>
          {badge && (
            <div className="badge bg-orange-500 text-orange-400 bg-opacity-20 font-light text-[0.6rem]">{badge}</div>
          )}
          <div className="text-neutral-400">·</div>
          <div className="text-neutral-400 opacity-90">{bpm} BPM</div>
        </div>
        {/* 2nd row */}
